...
#endif
int es8323_spk_con_gpio = 0;
int es8323_hp_det_gpio = 0;
int es8323_ear_con_gpio = 0;
int es8323_hp_mic_only = 0;
char es8323_mic_state = 0;
static int HP_IRQ=0;
static int hp_irq_flag = 0;
void mic_state_switch(void);

#define ES8323_HP_DET_ACTION_VALUE 1
...
static void switch_spk(int status)
{
    if (!gpio_is_valid(es8323_spk_con_gpio))
        return;
    else
        gpio_direction_output(es8323_spk_con_gpio,status);
}
...
static void es8323_codec_default(struct snd_soc_codec *codec)
{
...
static const struct snd_soc_dapm_route audio_map[] = {
    ...
    { "Left PGA Mux", "PGA", "Differential Mux" },

    { "Right PGA Mux", "Line 1", "RINPUT1" },
    { "Right PGA Mux", "Line 2", "RINPUT2" },
    { "Right PGA Mux", "PGA", "Differential Mux" },
...
static void on_off_ext_amp(int i)
{
    ...
    pr_debug("*** %s() SPEAKER set SPK_CON %d\n", __FUNCTION__, i);
    mdelay(50);
}
        ...
        unsigned int fmt)
{
    ...
    u8 adciface = 0;
    u8 daciface = 0;
    dev_dbg(codec_dai->dev,"%s----%d, fmt[%02x]\n",__FUNCTION__,__LINE__,fmt);
    ...
    switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
        case SND_SOC_DAIFMT_CBM_CFM:    // MASTER MODE
            dev_dbg(codec_dai->dev,"es8323 in master mode");
            iface |= 0x80;
            break;
        case SND_SOC_DAIFMT_CBS_CFS:    // SLAVE MODE
            dev_dbg(codec_dai->dev,"es8323 in slave mode");
            iface &= 0x7F;
            break;
        default:
            return -EINVAL;
        ...
        struct snd_soc_dai *dai)
{
    ...
    if (!es8323->is_startup) {
        ...
        dev_dbg(codec->dev,"%s-%d: ES8323_DACPOWER=%x\n",__FUNCTION__,__LINE__,
                snd_soc_read(codec, ES8323_DACPOWER));
    }

    dev_dbg(codec->dev,"Enter::%s----%d  es8323->sysclk=%d\n",__FUNCTION__,__LINE__,es8323->sysclk);
...
static int es8323_mute(struct snd_soc_dai *dai, int mute)
{
    ...
    dev_dbg(codec->dev,"Enter::%s----%d--mute=%d\n",__FUNCTION__,__LINE__,mute);
...
static int es8323_probe(struct snd_soc_codec *codec)
{
    ...
    if (gpio_is_valid(es8323_spk_con_gpio)) {
        ret = gpio_request(es8323_spk_con_gpio, "spk-con-gpio");
        if (ret != 0) {
            dev_err(codec->dev,"%s request SPK_CON error", __func__);
            return ret;
        }
    }
    switch_spk(1);

    if(gpio_is_valid(es8323_hp_det_gpio)) {
        ret = gpio_request(es8323_hp_det_gpio, "hp-det-gpio");
        if (ret != 0) {
            dev_err(codec->dev,"%s request HP_DET error", __func__);
            return ret;
        }
        ...
        if (HP_IRQ){
            ...
            if(ret == 0){
                dev_info(codec->dev,"%s:register ISR (irq=%d)\n", __FUNCTION__,HP_IRQ);
            }
            else 
                dev_err(codec->dev,"request_irq HP_IRQ failed\n");
        }
    }
...
static u32 strtol(const char *nptr, int base)
{
    ...
    {
        dev_err(es8323_codec->dev,"%s(): NULL pointer input\n", __FUNCTION__);
        return -1;
    }
    for(ret=0; *nptr; nptr++)
    {
        ...
        const char *_buf, size_t _count)
{
    ...
    if(!strncmp(_buf, "get", strlen("get")))
    {
        ...
        dev_info(dev,"%s(): get 0x%04x=0x%04x\n", __FUNCTION__, cur_reg, val);
    }
    else if(!strncmp(_buf, "put", strlen("put")))
    {
        ...
        if(p)
        {
            ...
            dev_info(dev,"%s(): set 0x%04x=0x%04x\n", __FUNCTION__, reg, val);
        }
        else
            dev_info(dev,"%s(): Bad string format input!\n", __FUNCTION__);
    }
    else
        dev_info(dev,"%s(): Bad string format input!\n", __FUNCTION__);
...
void mic_state_switch(void)
{
    dev_info(es8323_codec->dev,"%s %d\n",__FUNCTION__, es8323_mic_state);
    if(es8323_mic_state == 0) {
        if(ES8323_HP_DET_ACTION_VALUE == gpio_get_value(es8323_hp_det_gpio)){
            dev_info(es8323_codec->dev,"hp_det = 0,insert hp\n");
            dev_info(es8323_codec->dev,"hp mic use intern\n");
            snd_soc_write(es8323_codec, 0x0b,0x02);
        }else if(!(ES8323_HP_DET_ACTION_VALUE == gpio_get_value(es8323_hp_det_gpio))){
            dev_info(es8323_codec->dev,"hp_det = 1,deinsert hp\n");
            dev_info(es8323_codec->dev,"hp mic use extern\n");
...
int es8323_mic_state_ctrl(int cmd, char *ch) {
    ...
    if(cmd == MIC_STATE_INIT) {
        ...
        if(*ch >= '0' && *ch <= '2') {
            if (es8323_mic_state != (*ch-'0')) {
                es8323_mic_state = *ch - '0';
                mic_state_switch();
                dev_info(es8323_codec->dev,"es8323_mic_state=%d write: %s \n", es8323_mic_state ,ch);
            }
        }   
    } else if(cmd == MIC_STATE_READ) {
        *ch = es8323_mic_state + '0';
    }
    return 0;
}
        ...
        const struct i2c_device_id *id)
{
    ...
    if (es8323_spk_con_gpio < 0) {
        dev_warn(&i2c->dev,"%s() Can not read property spk-con-gpio\n", __FUNCTION__);
    }

    es8323_ear_con_gpio = of_get_named_gpio(i2c->dev.of_node, "ear-con-gpio", 0);
    if (gpio_is_valid(es8323_ear_con_gpio)) {
        ret = devm_gpio_request(&i2c->dev,es8323_ear_con_gpio,"ear-con-gpio");
        if (!ret)
            gpio_direction_output(es8323_ear_con_gpio,1);
        else
            dev_warn(&i2c->dev,"%s() es8323_ear_con_gpio request fail\n", __FUNCTION__);
    }
    else 
        dev_warn(&i2c->dev,"%s() Can not read property ear-con-gpio\n", __FUNCTION__);

    es8323_hp_det_gpio = of_get_named_gpio(i2c->dev.of_node, "hp-det-gpio", 0);
    if (es8323_hp_det_gpio < 0) {
        dev_warn(&i2c->dev,"%s() Can not read property hp-det-gpio\n", __FUNCTION__);
    }
    ...
    if (ret < 0){
        dev_err(&i2c->dev,"es8323 probe error\n");
        kfree(es8323);
        return ret;
    }

    dev_info(&i2c->dev,"es8323 probe i2c recv ok\n");

    ret =  snd_soc_register_codec(&i2c->dev,
            &soc_codec_dev_es8323, &es8323_dai, 1);
    if (ret < 0) {
        dev_err(&i2c->dev,"es8323 soc register fail\n");
        kfree(es8323);
        return ret;
    }
    ...
    if(es8323_hp_mic_only == 1) {
        ...
        if (ret < 0)
            dev_err(&i2c->dev,"failed to add dev_attr_mic_state file\n");  
